...
public class MessageController implements CommunityConstant{
    ...
    public String getNoticList(Model model) {
        ...
        // 查询未读通知的数量
        int unreadNoticeCount = messageService.findNoticeCount(user.getId(), null);
        model.addAttribute("unreadNoticeCount", unreadNoticeCount);
        return "/site/notice";
    }

    @RequestMapping(path = "/notice/detail/{topic}", method = RequestMethod.GET)
    public String getNotictDetail(@PathVariable("topic")String topic,Page page,Model model) {
        User user = hostHolder.getUser();
        page.setLimit(5);
